...
#ifndef SCMNOTE_TEMPO_EVENT_HPP
#define SCMNOTE_TEMPO_EVENT_HPP
...
namespace scmnote
{
	...
	public:
		TempoEvent();
		TempoEvent( unsigned long tempo );
		TempoEvent( const TempoEvent& te );
		virtual ~TempoEvent();
		virtual const TempoEvent& operator = ( const TempoEvent& se );

		virtual EnumMetaEvent Get_Type() const;

		virtual void Set_Tempo( unsigned long tempo );
		virtual unsigned long Get_Tempo() const;
